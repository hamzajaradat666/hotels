  ...
  const prepareEmptyHotelsJSX = () => {
    switch (store.emptyListStatus) {
      case EmptyListStatus.NoSearch:
        return <div className="flex justify-center items-center mt-16 col-span-2 px-32 border-2 border-gray-100 text-gray-500 w-full mb-2 rounded" style={{ fontSize: "2em" }}>Please search for date of reservasion.</div>
      case EmptyListStatus.Fetching:
        return <div className="flex justify-center items-center mt-16 col-span-2 px-32 border-2 border-gray-100 text-gray-500 w-full mb-2 rounded"><Loader></Loader></div>
      case EmptyListStatus.EmptyFilter:
        return <div className="flex justify-center items-center mt-16 col-span-2 px-32 border-2 border-gray-100 text-gray-500 w-full mb-2 rounded" style={{ fontSize: "2em" }}>No hotels found. :(</div>
      ...
      <div className="border-2 border-gray-500 rounded w-full mt-3">
        ...
        <div className="grid xs:grid-cols-2 sm:grid-cols-12 my-5 p-2">
          ...
          <div className="mx-2 w-10/12 col-auto sm:col-span-3">
            <button onClick={() => { sortByName() }} className={`p-2 border-2 border-gray-500 rounded w-full " ${nameFilter.enabled ? "bg-white" : "bg-gray-300"} `}>
          ...
          <div className="mx-2 w-10/12 col-auto sm:col-span-3">
            <button onClick={() => { sortByPrice() }} className={`p-2 border-2 border-gray-500 rounded w-full  ${priceFilter.enabled ? "bg-white" : "bg-gray-300"}`}>
              <div className="flex justify-around">
        ...
        <div className="grid xs:grid-cols-1 sm:grid-cols-3 ">
          ...
          {store.filteredHotels.length ?
            <div className="grid xs:grid-cols-2 sm:grid-cols-2 col-span-2 overflow-auto h-full h-72">{prepareHotelsJSX()}</div> :
            <div className="grid xs:grid-cols-2 sm:grid-cols-2 col-span-2 overflow-auto h-full">{prepareEmptyHotelsJSX()}</div>}
